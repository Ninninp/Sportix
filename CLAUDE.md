...
**J1 à J6 : codés. J1, J2, J3, J5 et J6 validés par l'utilisateur ; J4 en attente d'une seule vérification (le son).**
...
- **J6 (calendrier des blocs)** — codé et **validé le 22/09/2026** (détails plus bas). `/code-review` fait le 22/09/2026 : 2 bugs de double appui, corrigés — « Créer le bloc » créait deux blocs (le formulaire fixe maintenant l'identifiant du nouveau bloc à l'ouverture et `saveBlock` crée ou remplace ce bloc-là), et « Créer l'objectif » deux objectifs (verrou `useRef` dans `NameSheet`, qui protège aussi « Créer le programme » et « Nouveau jour ») ; **maquettes faites et publiées le 21/09/2026** (canvas « Sportix — Maquettes J6 », lien dans `design/README.md`, sources `design/maquettes-j6/`, 7 écrans × 2 thèmes) ; **décisions prises** : l'onglet Calendrier est une **vue mensuelle** et elle seule (la variante « frise des blocs » est écartée) ; **un bloc commence un lundi et dure un nombre entier de semaines** ; le deload est facultatif (« Aucun »). **Pas de date-fns** : les calculs tiennent en `Date` natif, comme `src/lib/week.ts` (plan mis à jour). Fait le 22/09/2026 : `src/lib/blocks.ts` testé (`blocks.test.ts`), Dexie **version 5** (`blocks`, `blockGoals`, objectif d'exemple « Force » ajouté aussi aux bases existantes), `src/db/blocks.ts` (CRUD, objectifs ; `createBlock` / `updateBlock` avec `shiftNext` pour décaler les blocs suivants, calcul dans `shiftNextBlocks`), `blockId` posé par `startSession` et `startProgramSession`. **Règle du rattachement** : le `blockId` d'une séance suit toujours les dates (bloc en cours à son démarrage) ; il est recalculé pour toutes les séances à chaque création / modification / suppression d'un bloc (`reattachSessions`, dans la même transaction). Test de migration v4 → v5 dans `src/db/blocks.test.ts`.

**Écrans du J6 codés et validés le 22/09/2026** : onglet Calendrier (`CalendarPage` : grille du mois, colonne S1/S2/D, mois précédent / suivant ; légende « séance » seulement s'il y a une séance dans le mois ; en bas, le bloc en cours ou le prochain dans une carte ordinaire entièrement touchable : pastille de couleur, nom, dates, barre des semaines, « Semaine 2/5 · 4/15 séances », choisie le 22/09/2026 à la place de la grande carte inversée). **Couleurs des blocs** (22/09/2026) : `block.color` (7 teintes, tokens `block-*` de `design/tokens.md`, champ facultatif non indexé, « sable » par défaut), proposée différente du dernier bloc (`suggestColor`), choisie par le carré à droite du nom dans le formulaire ; un jour commun à deux blocs est coupé en diagonale (`monthGrid` donne `colors` = tous les blocs du jour, `cellBackground`), légende une entrée par bloc. En sombre, teintes moyennes et claires avec texte foncé (les premières, sourdes, ne se voyaient pas) : tout ce qui se pose sur un bloc utilise `on-block` / `on-block-muted`, jamais `text` / `border-strong`. **Le formulaire d'un bloc ne doit pas défiler** : onglets masqués sur `/calendrier/nouveau` et `/calendrier/:id/modifier` (`App.tsx`), pastilles d'objectif et de programme sur une ligne qui défile à l'horizontale ; vérifié à 390×763 (hauteur utile d'un iPhone 14). **La page ne glisse jamais à l'horizontale** : `overflow-x-hidden` sur le conteneur de `App.tsx` (il était en `overflow-x: auto` à cause de l'`overflow-y-auto`), et le champ « Premier jour » n'affiche plus le champ date d'iOS : la case montre la date en toutes lettres et le vrai `input type=date` est posé dessus, transparent, dans une case `overflow-hidden` (affiché tel quel, Safari iOS lui donnait une largeur à lui qui faisait glisser la page, même avec `appearance-none`), `BlockFormPage` (nouveau / modifier, panneau « Nouvel objectif » ; **le deload ne se règle que là** : un seul par bloc, « Aucun » ou « sem. N » ; avertissement de chevauchement avec « Décaler « X » » / « Laisser le chevauchement » / « Changer les dates »), `BlockDetailPage` (tuiles, semaines, suppression ; plus de « + Semaine de deload », retiré le 22/09/2026 à la demande de l'utilisateur), ligne du bloc sur l'accueil (`WeekBar`). Parcours complet piloté dans Chrome (puppeteer-core hors dépôt), thèmes clair et sombre.
...
**Reprendre ici** :
1. Reste du J6 (parcours § 3.6) : les séances de deload ne déclenchent pas la double progression.
2. Reporter dans le canvas J6 ce qui a changé depuis sa publication (bouton de deload retiré, réglage « Par semaine », couleurs des blocs, carte du bas, formulaire sans onglets, champ date en toutes lettres, chiffres des réglages − / +).
3. Puis J7 (stats et progression).
...
- **Écritures concurrentes (relectures du J5 et du J6)** : deux appuis rapprochés sur un bouton sont la règle sur iPhone, pas l'exception. Une création doit être répétable sans doublon (identifiant fixé à l'ouverture de l'écran, comme `saveBlock`) ou verrouillée par un `useRef` (un état React arrive trop tard : deux appuis dans la même fraction de seconde passent tous les deux, vérifié dans Chrome). Un test du genre « existe-t-il déjà ? » doit donc être **dans la même transaction Dexie** que l'écriture (`startSession`, `startProgramSession`), et un bouton − / + doit calculer sa nouvelle valeur **depuis la base** (`changeDayExercise`), jamais depuis la valeur affichée, qui a un cycle de retard.
...
- **Petit réglage − / + (`MiniStepper`)** : panneau d'exercice de programme et formulaire de bloc. Chiffres comme les autres pavés (30 px, `tracking-[-0.02em]`), unité à part en petit gris (`unit`, comme « kg »), valeur sans chiffre (« Aucun ») en texte normal ; `valueWidth` commun à toutes les lignes d'un même écran pour aligner les boutons (aligné le 22/09/2026 à la demande de l'utilisateur).
