...
**Reprendre ici : J4 (minuteur de repos)** : maquettes déjà validées en D5, plus l'animation « +15 s » notée dans `design/README.md`, et l'écran de Réglages (repos par défaut, son, pas de charge par variante) prévu avec ce jalon. Frictions relevées lors de l'analyse de séance, **corrigées le 21/09/2026** (pas encore testées sur l'iPhone) : confirmation avant de terminer, boutons « + Série » / « + Exercice » en bas quand un exercice est fini, saisie au clavier en touchant le nombre du pavé (`onType` de `NumberStepper`, lecture par `parseWeight` / `parseReps`), « N séries » sans objectif, la première fois n'est plus un record (`src/lib/records.ts`). Liste dans `design/README.md`.
