...
**J4 (minuteur de repos) codé le 21/09/2026**, pas encore validé : repos lancé à la validation d'une série, écrans repos actif / prolongé / terminé, « +15 s » (animé depuis l'écran de fin), barre « Séance en cours » au-dessus des onglets, son, écran gardé allumé, vibration (Android), Réglages (repos par défaut, son, pas de charge par variante). Vérifié en local (82 tests ; cycle complet piloté dans Chrome, thèmes clair et sombre). Premier test iPhone (21/09/2026) : écran allumé OK, décompte juste après verrouillage OK. Corrigé ensuite : texte « Son à la fin · l'écran reste allumé » retiré ; secondes irrégulières (chronos rafraîchis pile au changement de seconde, `useNow(anchor)` + `msUntilNextSecond`) ; animations de fin de repos et « +15 s » refaites en CSS (les View Transitions ne marchaient pas sur son iPhone). Re-test : secondes régulières OK. Bande verte restée en haut après « +15 s de repos » corrigée (le voile qui s'efface est retiré de la page à la fin de son animation : iOS colore la zone de la barre d'état d'après les éléments fixes du haut, même transparents). Les animations seront reprises au **J10** (ajouté au plan le 21/09/2026, à faire en dernier). Voile réduit à 250 ms (bande validée). `/code-review` du J4 fait le 21/09/2026 : pas de bug sérieux ; corrigé : écran allumé 2 min au plus après la fin du repos (plus indéfiniment), son déverrouillé par n'importe quel appui (sinon muet après un rechargement en plein repos). Reste : test du son. **J5 (programmes) codé le 21/09/2026**, pas encore validé : maquettes validées le 21/09/2026 (canvas « Sportix — Maquettes J5 », lien dans `design/README.md`, sources `design/maquettes-j5/` générées par `design/maquettes-d5/generer.mjs`) ; onglet Programmes, accueil façon Lyfta, séance « en liste ». Vérifié en local (108 tests ; parcours complet piloté dans Chrome : programme à 2 jours, accueil, « Autre séance », séance du jour, repos du programme, récap, rotation A → B, historique). **Reprendre ici** : test du J5 sur l'iPhone à la salle, test du son (J4), puis `/code-review` du J5, puis J6 (calendrier des blocs).
