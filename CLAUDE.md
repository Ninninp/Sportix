...
- **J6 (calendrier des blocs)** — **maquettes faites et publiées le 21/09/2026** (canvas « Sportix — Maquettes J6 », lien dans `design/README.md`, sources `design/maquettes-j6/`, 7 écrans × 2 thèmes) ; **décisions prises** : l'onglet Calendrier est une **vue mensuelle** et elle seule (la variante « frise des blocs » est écartée) ; **un bloc commence un lundi et dure un nombre entier de semaines** ; le deload est facultatif (« Aucun »). **Pas de date-fns** : les calculs tiennent en `Date` natif, comme `src/lib/week.ts` (plan mis à jour). Code commencé : `src/lib/blocks.ts` (semaines d'un bloc, deload, chevauchements, formats français) — **écrit mais pas encore testé ni branché**.

**Reprendre ici (J6)** :
1. Tests de `src/lib/blocks.ts` (`blocks.test.ts`), puis la base : Dexie **version 5** (`blocks`, `blockGoals`) et `blockId` sur les séances (champ non indexé, comme `programDayId`), `src/db/blocks.ts`.
2. Les écrans, d'après le canvas J6 : `src/features/blocks/` (CalendarPage = la grille du mois, BlockFormPage, BlockDetailPage), route `/calendrier` à la place du `ComingSoon` de `src/routes.tsx`, ligne du bloc sur l'accueil **sous** les chiffres de la semaine.
3. Rattachement : `startSession` et `startProgramSession` posent le `blockId` du bloc en cours (`blockIdFor`).
4. Puis `/code-review` du J6, test sur l'iPhone, validation.
