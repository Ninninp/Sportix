...
**J1 en attente d'une décision** : le message iOS du mode avion reste. Mesuré le 21/09/2026 : une fois l'app installée, la seule requête réseau à l'ouverture est `sw.js`, faite par le navigateur lui-même (même JavaScript désactivé : vérification de mise à jour prévue par la norme des service workers). Impossible à supprimer depuis l'app ; n'apparaît qu'en mode avion (pas avec les données mobiles activées sans réseau). Si l'utilisateur accepte cette limite → cocher J1.

**Reprendre ici : J4 (minuteur de repos)** : maquettes déjà validées en D5, plus l'animation « +15 s » notée dans `design/README.md`, et l'écran de Réglages (repos par défaut, son, pas de charge par variante) prévu avec ce jalon. Frictions relevées lors de l'analyse de séance, à discuter avec l'utilisateur :
- Après la 1ʳᵉ série d'un exercice nouveau (1 seule série créée), le gros bouton du bas devient « Terminer la séance » ; « + Série » est petit, en haut, hors de la zone du pouce. Risque de terminer par erreur.
- « Terminer » (en-tête) et « Terminer la séance » ferment sans confirmation.
- « 1 × — reps » quand il n'y a pas d'objectif ; à la 1ʳᵉ séance, chaque exercice compte comme un record.
