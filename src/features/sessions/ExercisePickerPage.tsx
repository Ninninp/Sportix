...
// Quand l'exercice accepte plusieurs variantes, la ligne se déplie pour la choisir.
...
function ExercisePickerPage() {
  ...
  if (!session || exercises === undefined || history === undefined) return null

  async function choose(exercise: Exercise, variant: Variant | null) {
    ...
    <main className="flex min-h-0 flex-1 flex-col gap-3 px-4 pt-1 pb-4">
      ...
      {found.length === 0 ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-2 px-4 text-center">
          ...
          <p className="text-body text-muted">Crée-le : il sera ajouté à la séance et proposé ensuite partout.</p>
        </div>
      ) : (
              ...
              return (
                <div key={e.id} className={open ? 'bg-surface-2' : ''}>
                  <div className="flex min-h-16 items-center gap-3 py-2 pr-2 pl-4">
                    <button
                      type="button"
                      onClick={() => (e.variants.length > 1 ? setExpanded(open ? null : e.id) : choose(e, e.variants[0] ?? null))}
                    ...
                    {e.variants.length <= 1 && (
                      <button
                        ...
                        onClick={() => choose(e, e.variants[0] ?? null)}
