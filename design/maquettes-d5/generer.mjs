...
const nav = (t, active, links = {}) => {
  ...
  return `<nav aria-label="Navigation principale" style="flex-shrink: 0; height: 56px; display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); background: ${t.surface}; border-top: 1px solid ${t.border};">${tabs
    .map(([k, l, i, h]) => `<a href="${h}"${k === active ? ' aria-current="page"' : ''}${links.dot === k ? ` aria-label="${l}, sauvegarde à faire"` : ''} style="${links.dot === k ? 'position: relative; ' : ''}display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; font-size: 12px; font-weight: ${k === active ? 700 : 500}; color: ${k === active ? t.text : t.muted}; text-decoration: none;"><span style="width: 24px; height: 3px; border-radius: 2px; background: ${k === active ? t.text : 'transparent'};"></span>${ic(i)}<span>${l}</span>${links.dot === k ? `<span aria-hidden="true" style="position: absolute; top: 7px; left: calc(50% + 7px); width: 10px; height: 10px; border-radius: 5px; background: ${t.danger}; box-shadow: 0 0 0 2px ${t.surface};"></span>` : ''}</a>`)
    .join('')}</nav>`;
};
...
const progExSheet = (t, dp) => sheet(t, 'Squat dans Force A — Jambes',
  ...
  `<label style="display: flex; align-items: center; justify-content: space-between; gap: 12px; min-height: 64px;"><span style="font-size: 17px;">Double progression</span><input class="sw" type="checkbox" role="switch"${dp ? ' checked' : ''}></label>` +
...
const weekDots = (t) => `<div style="display: flex; justify-content: center; gap: 25px; margin-top: 6px;">${['L', 'M', 'M', 'J', 'V', 'S', 'D']
  .map((d, i) => {
...
S['Jour-choix'] = { title: 'Choisir un autre jour (panneau)', page: 'j5', render: (t) => frame(t, { navActive: 'seance', navLinks: J5NAV(t), main: homeJ5(t),
  overlay: sheet(t, 'Quelle séance aujourd’hui ?',
    ...
    card(t, dayOption(t, 'Force A — Jambes', '', true, true) + dayOption(t, 'Force B — Haut du corps', '', false), 'overflow: hidden;') +
    card(t, dayOption(t, 'Séance libre', '', false, true), 'overflow: hidden;') +
...
// ----- Séance « en liste », inspirée de Lyfta mais à une main (exemple du 21/09/2026) -----
// Toute la séance dans une liste qui défile : l'exercice en cours est déplié en tableau
// N° | Dernière fois | Kg | Reps | ✓, les suivants sont repliés sur une ligne. Le pavé − / + et
// « Valider la série » restent fixés en bas, dans la zone du pouce. Remplace les pastilles, le
// sous-titre, la barre de progression et « Ensuite ».
P.check = '<path d="M5 12.5l4.5 4.5L19 7.5"></path>';
const COLS = 'grid-template-columns: 28px minmax(0, 1fr) 76px 48px 36px; column-gap: 8px;';
const listSet = (t, n, last, kg, reps, state) => {
  const st = state === 'done' ? `background: ${t.surface2}; color: ${t.muted};` : state === 'now' ? `background: ${t.inverse}; color: ${t.onInverse};` : `color: ${t.faint};`;
  const tick = state === 'done'
    ? `<span aria-label="faite" style="width: 28px; height: 28px; border-radius: 14px; background: ${t.text}; color: ${t.bg}; display: flex; align-items: center; justify-content: center;">${ic('check', 18, 3)}</span>`
    : `<span aria-hidden="true" style="box-sizing: border-box; width: 28px; height: 28px; border-radius: 14px; border: 1.5px ${state === 'now' ? `solid ${t.onInverse}` : `dashed ${t.strong}`};"></span>`;
  return `<div role="row" aria-label="Série ${n}"${state === 'now' ? ' aria-current="true"' : ''} style="display: grid; ${COLS} align-items: center; min-height: 52px; padding: 0 10px; border-radius: 10px; ${st}"><span class="num" style="font-size: 15px;">${n}</span><span class="num" style="font-size: 15px; font-weight: 500; ${state === 'now' ? `color: ${t.onInverseMuted};` : ''}">${last}</span><span class="num" style="font-size: 22px; text-align: right;">${kg}</span><span class="num" style="font-size: 22px; text-align: right;">${reps}</span><span style="display: flex; justify-content: flex-end;">${tick}</span></div>`;
};
// Pavé − / + compact : bloc de 52 px (≥ 48 px au doigt), valeur en 30 px, étiquette en 11 px
const stepperS = (t, label, value, unit, aria, minus, plus) =>
  `<div role="group" aria-label="${aria}" style="display: flex; align-items: stretch; height: 52px; border: 1.5px solid ${t.strong}; border-radius: 12px; overflow: hidden; background: ${t.surface};"><button aria-label="${minus}" style="box-sizing: border-box; width: 56px; flex-shrink: 0; border: 0; background: ${t.surface2}; color: ${t.text}; font: inherit; font-size: 24px; font-weight: 700; cursor: pointer;">−</button><div style="flex-grow: 1; display: flex; flex-direction: column; align-items: center; justify-content: center;"><span style="font-size: 11px; line-height: 13px; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; color: ${t.muted};">${label}</span><span><span class="num" style="font-size: 30px; line-height: 32px; letter-spacing: -0.02em;">${value}</span> <span style="font-size: 13px; font-weight: 600; color: ${t.muted};">${unit}</span></span></div><button aria-label="${plus}" style="box-sizing: border-box; width: 56px; flex-shrink: 0; border: 0; background: ${t.surface2}; color: ${t.text}; font: inherit; font-size: 24px; font-weight: 700; cursor: pointer;">+</button></div>`;
const folded = (t, name, info) =>
  `<a href="#" style="flex-shrink: 0; box-sizing: border-box; display: flex; align-items: center; gap: 12px; min-height: 56px; padding: 0 8px 0 16px; border-radius: 16px; background: ${t.surface}; border: 1px solid ${t.border}; color: ${t.text}; text-decoration: none;"><span style="flex-grow: 1; font-size: 17px; font-weight: 600;">${name}</span><span class="num" style="font-size: 15px; font-weight: 500; color: ${t.muted};">${info}</span><span style="display: flex; color: ${t.muted};">${ic('chevD', 20)}</span></a>`;

S['Seance-liste'] = { title: 'Séance · en liste (exemple inspiré de Lyfta)', page: 'j5', render: (t) => frame(t, { pad: '4px 16px 16px', main:
  seanceHeader(t, '24:10').replace('>Séance libre<', '>Force A — Jambes<') +
  `<div style="flex-grow: 1; min-height: 0; overflow: hidden; display: flex; flex-direction: column; gap: 8px;">` +
    card(t, `<div style="display: flex; align-items: center; gap: 8px; padding: 6px 0 2px 6px;"><div style="flex-grow: 1; display: flex; align-items: baseline; gap: 8px;"><span style="font-size: 20px; line-height: 24px; font-weight: 800;">Squat</span><span class="num" style="font-size: 15px; color: ${t.muted};">4–6 reps</span></div>${iconBtn(t, 'more', `Options de l'exercice`, '#')}</div>` +
      `<div role="row" style="display: grid; ${COLS} padding: 0 10px; font-size: 11px; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; color: ${t.muted};"><span>N°</span><span>Dernière fois</span><span style="text-align: right;">Kg</span><span style="text-align: right;">Reps</span><span></span></div>` +
      listSet(t, 1, '100 × 5', '102,5', 5, 'done') + listSet(t, 2, '100 × 5', '102,5', 5, 'now') + listSet(t, 3, '100 × 4', '102,5', 5, 'next') +
      `<button style="align-self: flex-start; min-height: 44px; padding: 0 6px; border: 0; background: transparent; color: ${t.text}; font: inherit; font-size: 15px; font-weight: 600; cursor: pointer;">+ Série</button>`,
      'flex-shrink: 0; padding: 6px; display: flex; flex-direction: column; gap: 4px;') +
    folded(t, 'Presse à cuisses', '0/3') + folded(t, 'Leg curl', '0/3') + folded(t, 'Fentes bulgares', '0/3') + folded(t, 'Mollets debout', '0/4') +
  `</div>` +
  card(t, `<div style="display: flex; justify-content: space-between; align-items: center;"><span style="font-size: 15px; font-weight: 700;">Squat · série 2</span>${badgeUp(t, 'charge +2,5 kg')}</div>` +
    stepperS(t, 'Charge', '102,5', 'kg', 'Charge', 'Retirer 2,5 kg', 'Ajouter 2,5 kg') +
    stepperS(t, 'Reps', '5', 'reps', 'Répétitions', 'Retirer une rep', 'Ajouter une rep'),
...
const weekBar = (t, { weeks, current, deload, done = [], onInverse = false }) => {
  const line = onInverse ? t.onInverseMuted : t.strong;
  const fill = onInverse ? t.onInverse : t.text;
  const txt = onInverse ? t.inverse : t.bg;
  const seg1 = (i) => {
    const n = i + 1;
    const isD = n === deload;
    const box = 'box-sizing: border-box; height: 18px; border-radius: 9px; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 800;';
    if (n === current) return `<span style="${box} background: ${fill}; color: ${txt};">S${n}</span>`;
    if (done.includes(n)) return `<span style="${box} background: ${line};"></span>`;
    return `<span style="${box} color: ${onInverse ? t.onInverseMuted : t.muted}; border: 1.5px ${isD ? 'dashed' : 'solid'} ${line};">${isD ? 'D' : ''}</span>`;
  };
...
S['Calendrier-vide'] = { title: 'Calendrier · aucun bloc', page: 'j6', render: (t) => frame(t, { navActive: 'cal', navLinks: J6NAV(t), main:
  ...
  card(t, `<span style="font-size: 20px; font-weight: 800;">Aucun bloc pour l’instant</span><span style="font-size: 15px; line-height: 20px; color: ${t.muted};">Un bloc, c’est un objectif sur quelques semaines (« Force, 5 semaines »). Tes séances s’y rattachent toutes seules : tu pourras comparer un bloc à l’autre.</span>`,
...
// ---------- Fiche d'un bloc ----------
const weekRow = (t, { n, dates, sessions, state }) => {
  const now = state === 'now';
  const deload = state === 'deload';
  return `<div style="box-sizing: border-box; display: flex; align-items: center; gap: 12px; min-height: 52px; padding: 0 12px; border-radius: 10px; ${now ? `background: ${t.inverse}; color: ${t.onInverse};` : deload ? `border: 1.5px dashed ${t.strong};` : ''}">` +
    `<span class="num" style="width: 28px; font-size: 15px; font-weight: 800; color: ${now ? t.onInverse : t.muted};">${deload ? 'D' : `S${n}`}</span>` +
    `<span class="num" style="flex-grow: 1; font-size: 15px; ${now ? '' : `color: ${t.muted};`}">${dates}</span>` +
...
S['Objectif-nouveau'] = { title: 'Nouvel objectif (panneau)', page: 'j6', render: (t) => frame(t, { gap: 16,
  ...
  overlay: sheet(t, 'Nouvel objectif',
    ...
    `<div style="display: flex; flex-direction: column; gap: 8px;">${btn.pri(t, 'Créer l’objectif', file(t, 'Bloc-nouveau'), 'width: 100%;')}${btn.sec(t, 'Annuler', file(t, 'Bloc-nouveau'), 'width: 100%;')}</div>`) }) };
...
// ===================================================================================
P.upload = '<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3"></path>'; // flèche qui entre (reprise du code, 25/09/2026)
...
// Rappel (retouche du 25/09/2026) : pas de ligne sur l'accueil, qui reste épuré ; une pastille
// sur l'onglet Réglages quand la dernière sauvegarde date de plus de 30 jours.
S['Accueil-rappel'] = { title: 'Rappel : pastille sur l’onglet Réglages (plus de 30 jours)', page: 'j8', render: (t) => frame(t, { navActive: 'seance', navLinks: { ...J8NAV(t), dot: 'reglages' },
  main: homeJ5(t,
